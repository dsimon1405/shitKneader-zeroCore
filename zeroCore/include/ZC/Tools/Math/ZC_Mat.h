...
namespace ZC_Mat
{
    ...
    */
    ...
    ZC_Mat4<TValue> Perspective(TValue fovy, TValue aspect, TValue nearPlane, TValue farPlane) noexcept
    {
        ...
        TValue radFovy = ZC_Vec::Radians(fovy);
        TValue tanHalfFovy = tan(radFovy / static_cast<TValue>(2));
    ...
    up - a normalized direction pointing to the top of your world. Typically, in the RIGHT coordinate system it is assumed to be (0,0,1), in the LEFT (0,0,-1).
    
    Return:
    View matrix.
    */
    template<ZC_cVecTypes TValue>
    ZC_Mat4<TValue> LookAt(const ZC_Vec3<TValue>& position, const ZC_Vec3<TValue>& lookOn, const ZC_Vec3<TValue>& up) noexcept
    {
